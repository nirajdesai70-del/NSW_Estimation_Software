...
class TestFixedNoDiscountRatePreservation:
    ...
    def test_fixed_no_discount_requires_override_rate(self, resolver):
        ...
        with pytest.raises(
            ValueError, match="override_rate is required for FIXED_NO_DISCOUNT"
        ):
...
class TestFixedNoDiscountInComputePricing:
    ...
    async def test_fixed_no_discount_skips_discount_rules(self, monkeypatch):
        ...
        monkeypatch.setattr(
            quotation_ep.DiscountRuleLookup, "__init__", lambda self, db: None
        )
        monkeypatch.setattr(
            quotation_ep.DiscountRuleLookup,
            "list_active_rules_for_quote",
            lambda self, **kwargs: [],
        )
        ...
        assert subtotal >= Decimal(
            "300"
        ), "Subtotal must include fixed line at full rate"
...
class TestIsLockedEnforcement:
    ...
    async def test_delete_unlocked_item_succeeds(self, monkeypatch):
        ...
        # Patch AuditLogger
        monkeypatch.setattr(
            quotation_ep.AuditLogger, "log_event", lambda **kwargs: None
        )
