        ...
        *,
        ...
        if has_make_series_rules:
            if sku_id is None:
                ...
                make_id, series_id = self.sku_series_lookup.get_make_series_for_sku(
                    sku_id=sku_id
                )
                if make_id and series_id:
                    ...
                    for rule in rules:
                        if (
                            rule.scope_type == DiscountScope.MAKE_SERIES
                            and rule.scope_key == make_series_key
                        ):
                            return DiscountDecision(
        ...
        if category_id:
            ...
            for rule in rules:
                if (
                    rule.scope_type == DiscountScope.CATEGORY
                    and rule.scope_key == category_key
                ):
                    return DiscountDecision(
                        effective_discount_pct=qpct(rule.discount_pct),
        ...
        for rule in rules:
            if (
                rule.scope_type == DiscountScope.SITE
                and rule.scope_key == SCOPE_KEY_SITE
            ):
                return DiscountDecision(
                    effective_discount_pct=qpct(rule.discount_pct),
                    applied_scope="SITE",
