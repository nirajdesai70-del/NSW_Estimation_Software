...
**Status:** LOCKED – ENGINEERING CONSISTENT (v1.2)  
**Date:** 2025-01-XX  
...
- **Version:** 1.2  
**Note:** v1.2 – SC_L alignment corrected, Duty (AC1/AC3) clarified as SC_L3 with ratings as attributes, no semantic conflicts remaining.
...
**Duty class (AC1 / AC3) belongs to SC_L3 (Feature Class).**  
**Duty-specific ratings (Current A, kW, HP) are stored as KVU attributes on L1 lines.**  
**Duty does NOT create additional L2 rows unless the OEM Catalog No changes.**
...
> ⚠️ **IMPORTANT**  
> OEM catalog tables may show separate AC1 / AC3 rows for the SAME SKU.  
> These rows represent engineering ratings, NOT commercial SKU separation.  
> Only a change in OEM Catalog No creates a new L2 row.
...
|------|----------------|-------------------|----------------------------------------|---------------------------|-----------------|
| 8 | Contactor (LC1E) | Yes | Distinct SKUs only (AC1/AC3 as SC_L3 unless SKU changes) | Yes | |
...
## 16. 🔒 FINAL FREEZE STATEMENT (v1.2)

**L2 rows are created ONLY per distinct OEM Catalog No (Make + OEM_Catalog_No).**

• AC1 / AC3 duty differences create separate L1 engineering lines, NOT separate L2 SKUs  
• Frame is derived from rating bands and NEVER multiplies  
• Pole count multiplies ONLY when OEM SKU changes  
• Voltage creates new L2 rows ONLY when SKU suffix changes  
• Accessories are SC_L4 and NEVER multiply base SKUs  

**This rule is FINAL and applies uniformly to all Schneider pages (8 → end).**
...
**Document Status:** ✅ **READY FOR REVIEW** (v1.2 - SC_L alignment + duty clarification corrected)

**Critical Updates in v1.2:**
- ✅ L2 generation rule corrected: No combinatorial multiplication
- ✅ Frame handling corrected: Derived, not multiplier
- ✅ Duty handling corrected: SC_L3 (Feature Class), not attribute; duty-specific ratings are KVU attributes
- ✅ Poles placement corrected: SC_L1 only (not SC_L2)
