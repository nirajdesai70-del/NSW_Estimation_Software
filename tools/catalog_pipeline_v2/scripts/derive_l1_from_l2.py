        ...
        for base_ref, group in l2_with_price.groupby('Base_Ref'):
            ...
            for duty in duties:
                for voltage_info in voltage_variants:
                    ...
                    # v1.2 CLEAN RULE: L1 generic descriptors must be vendor/series-neutral
                    # Do NOT include: series_bucket, make, oem_series, LC1E
                    desc_parts = []
                    if product_type:
                        desc_parts.append(product_type)
                    # REMOVED: series_bucket (violates v1.2 CLEAN - series must not appear in L1 generic fields)
                    ...
                    # REMOVED: kW from descriptor (ratings belong in KVU attributes, not generic descriptor)
                    # kW should be in L1_ATTRIBUTES_KVU, not in GenericDescriptor
