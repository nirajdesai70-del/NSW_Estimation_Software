...
def read_canonical_table(input_file):
    ...
    try:
        ...
        # Check if file has LC1E_COIL_CODE_PRICES sheet (LC1E canonical format)
        if 'LC1E_COIL_CODE_PRICES' in xls.sheet_names:
            print("✓ Found LC1E_COIL_CODE_PRICES sheet - using LC1E canonical format")
            df = pd.read_excel(input_file, sheet_name='LC1E_COIL_CODE_PRICES')
            print(f"✓ Read LC1E_COIL_CODE_PRICES: {len(df)} rows")
            df['_input_mode'] = 'LC1E_CANONICAL'
            return df
...
def expand_completed_skus(df):
    ...
    for idx, row in df.iterrows():
        if mode == 'LC1E_CANONICAL':
            # LC1E_CANONICAL mode: use completed_sku directly from LC1E_COIL_CODE_PRICES sheet
            # Also merge with LC1E_CANONICAL_ROWS for poles, frame, and other metadata
            completed_sku_col = None
            for col in ['completed_sku', 'Completed_OEM_Catalog_No', 'OEM_Catalog_No']:
                if col in df.columns:
                    completed_sku_col = col
                    break
            
            if not completed_sku_col:
                continue
            
            completed_sku = row.get(completed_sku_col)
            if pd.isna(completed_sku) or str(completed_sku).strip() == '':
                continue
            
            # Get price
            price_col = None
            for col in ['price', 'Price', 'Rate']:
                if col in df.columns:
                    price_col = col
                    break
            ...
            # Get base_ref_clean and coil_code
            base_ref_clean = row.get('base_ref_clean', '') if 'base_ref_clean' in df.columns else ''
            coil_code = row.get('coil_code', '') if 'coil_code' in df.columns else ''
            
            # Create expanded row
            new_row = row.copy()
            new_row['Completed_OEM_Catalog_No'] = str(completed_sku).strip()
            new_row['Base_Ref'] = base_ref_clean if pd.notna(base_ref_clean) and str(base_ref_clean).strip() else ''
            new_row['CoilCode'] = coil_code if pd.notna(coil_code) and str(coil_code).strip() else ''
            new_row['Price'] = float(price_value)
            
            # Lock metadata fields
            new_row['Make'] = 'Schneider'
            new_row['SeriesBucket'] = 'LC1E'
            new_row['OEM_Series_Range'] = 'Easy TeSys'
            new_row['Item_ProductType'] = 'Contactor'
            new_row['Business_SubCategory'] = 'Power Contactor'
            
            # Add voltage info
            if 'voltage_value' in df.columns and pd.notna(row.get('voltage_value')):
                new_row['Voltage'] = float(row.get('voltage_value'))
            elif coil_code and coil_code in COIL_CODE_MAP:
                new_row['Voltage'] = COIL_CODE_MAP[coil_code]['voltage']
            
            if 'voltage_type' in df.columns and pd.notna(row.get('voltage_type')):
                new_row['VoltageType'] = row.get('voltage_type')
            elif coil_code and coil_code in COIL_CODE_MAP:
...
def build_rating_map(expanded_df, source_file_name, input_file=None):
    """
    ...
    # For LC1E format, read ratings from LC1E_CANONICAL_ROWS sheet
    if input_file:
        try:
            xls = pd.ExcelFile(input_file)
            if 'LC1E_CANONICAL_ROWS' in xls.sheet_names:
                print("✓ Reading ratings from LC1E_CANONICAL_ROWS sheet")
                canonical_df = pd.read_excel(input_file, sheet_name='LC1E_CANONICAL_ROWS')
                
                # Extract ratings from canonical rows
                for idx, row in canonical_df.iterrows():
                    base_ref_clean = row.get('base_ref_clean')
                    if pd.isna(base_ref_clean) or str(base_ref_clean).strip() == '':
                        continue
                    
                    base_ref_clean = str(base_ref_clean).strip()
                    
                    # AC1 rating
                    ac1_current = row.get('ac1_current_a')
                    if pd.notna(ac1_current):
                        try:
                            ac1_val = float(ac1_current)
                            if ac1_val > 0:
                                rating_rows.append({
                                    ...
                                    'Aux_Contacts': f"{row.get('aux_no_count') or ''}NO+{row.get('aux_nc_count') or ''}NC" if pd.notna(row.get('aux_no_count')) or pd.notna(row.get('aux_nc_count')) else '',
                ...
                print(f"✓ Created RATING_MAP: {len(result)} rating rows from LC1E_CANONICAL_ROWS")
                return result
        except Exception as e:
            print(f"⚠ Warning: Could not read LC1E_CANONICAL_ROWS: {e}")
    
    # Fallback: Extract from expanded_df (original logic)
...
def main():
    ...
    # For LC1E_CANONICAL mode, merge with LC1E_CANONICAL_ROWS for poles, frame, and metadata
    input_mode = df['_input_mode'].iloc[0] if '_input_mode' in df.columns else 'RAW'
    if input_mode == 'LC1E_CANONICAL':
        try:
            canonical_rows_df = pd.read_excel(args.input, sheet_name='LC1E_CANONICAL_ROWS')
            # Merge on base_ref_clean
            if 'base_ref_clean' in canonical_rows_df.columns and 'Base_Ref' in expanded_df.columns:
                # Select only the metadata columns we want to merge
                merge_cols = ['base_ref_clean', 'poles', 'frame_label', 'ac1_current_a', 'ac3_current_a', 
                             'motor_hp', 'motor_kw', 'aux_no_count', 'aux_nc_count']
                merge_cols = [col for col in merge_cols if col in canonical_rows_df.columns]
                
                canonical_metadata = canonical_rows_df[merge_cols].drop_duplicates(subset=['base_ref_clean'])
                expanded_df = expanded_df.merge(
                    canonical_metadata,
                    left_on='Base_Ref',
                    right_on='base_ref_clean',
                    how='left',
                    suffixes=('', '_canonical')
                )
                # Clean up duplicate column
                if 'base_ref_clean_canonical' in expanded_df.columns:
                    expanded_df = expanded_df.drop(columns=['base_ref_clean_canonical'])
                print(f"✓ Merged LC1E_CANONICAL_ROWS metadata: {len(merge_cols)} fields")
        except Exception as e:
            print(f"⚠ Warning: Could not merge LC1E_CANONICAL_ROWS: {e}")
    ...
    # Build RATING_MAP
    rating_map = build_rating_map(expanded_df, source_file_name, input_file=args.input)
