...
Status: CANONICAL
Last_Updated: 2025-12-27T23:03:55.428814
KB_Path: phase5_pack/04_RULES_LIBRARY/governance/RAG_RULEBOOK.md
---
...
H3 gates are quality and control gates to prevent the Phase-5 knowledge base from drifting into "mostly WORKING noise" again.

### What H3 Protects

- **Authority integrity**: CANONICAL governance docs must always stay present and discoverable
- **Operational stability**: prevents accidental removal/renaming of key governance files or breaking scan paths
- **Audit readiness**: ensures decisions/policies remain traceable and not overwritten by drafts

### H3 Gate Checks

The H3 gate (`scripts/ci_h3_governance_gate.py`) enforces:

1. **Mandatory governance docs exist and are indexed**
   - `RAG_KB/RAG_RULEBOOK.md`
   - `RAG_KB/phase5_pack/08_PROMOTION_WORKFLOW.md`
   - `docs/PHASE_5/00_GOVERNANCE/PHASE_5_CHARTER.md`

2. **Canonical coverage floor**
   - `canonical_count >= 10` (tunable threshold)
   - Governance canonical must be at least 3 (rulebook + promotion + charter)

3. **No broken _ACTIVE_FILE.txt overrides**
   - Fail if any broken pointers are reported in `RISKS_AND_GAPS.md`

4. **Canonical files must have Version**
   - If `Status: CANONICAL` but missing `Version:` in front-matter → fail

### Thresholds and Adjustment

**Current thresholds:**
- `CANONICAL_FLOOR = 10` (minimum total canonical files)
- `GOV_CANONICAL_FLOOR = 3` (minimum canonical governance files)

**How to adjust safely:**
1. Check current counts in `00_INDEX.json` statistics
2. Update constants in `scripts/ci_h3_governance_gate.py`
3. Ensure adjusted floors reflect actual governance needs
4. Document threshold changes in this section

**When to add H3:**
- After CI is stable (already in place)
- Once the team starts pushing lots of Phase-5 edits
- When governance drift becomes a concern

These gates keep governance stable without blocking normal WORKING iterations.

---

## Change Log

- **v1.1** (2025-12-27): Added H3 governance gates section with thresholds and adjustment guidance
